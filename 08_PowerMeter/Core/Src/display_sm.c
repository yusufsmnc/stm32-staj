	...
	switch (dsm->current){
		...
		case DISP_STATE_VRMS_IRMS:
			...
			LCD_Print_Padded(lcd, "V:%d.%03d I:%d.%03d",(int)meter->Vrms,
					(int)((meter->Vrms - (int)meter->Vrms) * 1000.0f));
			LCD_Set_Cursor(lcd, 1, 0);
		...
		case DISP_STATE_PF:
			...
			LCD_Set_Cursor(lcd, 1, 0);
			LCD_Print_Padded(lcd, "Q:%c%d.%03d VAr", (meter->P_react < 0) ? '-' : '+',
					(int)((meter->P_react < 0) ? -meter->P_react : meter->P_react),
					(int)(((meter->P_react < 0) ? -meter->P_react : meter->P_react) -
							(int)((meter->P_react < 0) ? -meter->P_react : meter->P_react)) * 1000);
			break;
		default:
			DisplaySM_Init(dsm);
			break;
