...
void Generate_Waves(float faz_aci){
	float fazKaymaMiktari = faz_aci * (float)M_PI / 180.0f;
...
void HAL_TIM_PeriodElapsedCallback(TIM_HandleTypeDef *htim){
    if(htim->Instance == TIM2)
        sysSM.lcdUpdateFlag = true;
    else if(htim->Instance == TIM4)
        sysSM.uartTransmitFlag = true;
}
